...
const formSection = (req,res)=>{
    ...
    return res.end();
        ...
        req.on("end",()=>{
            ...
            console.log(measage);
            fs.writeFile("app/message.txt",measage, (error)=>{
                if(error){
                    res.statusCode = 302
        ...
        fs.readFile("app/error.txt", (err, data) => {
            if(err){
                console.log(err);
            }
            res.end(data);
        ...
        fs.readFile("app/success.txt",(err,data)=>{
            if(err){
                console.log(err);
            }
            res.end(data)
